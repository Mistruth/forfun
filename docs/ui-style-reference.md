...
本文档定义通用 Web 项目的视觉语言和交互规范。所有 UI 开发以此为准。
...
本节是对原规范的补充，不改变前文风格方向。后续 Web 项目实现 UI 时，建议把这些作为额外验收项。

### 12.1 适用范围

- 本规范默认适用于信息密度较高的工具型 Web 产品。
- 如果项目是营销页、品牌官网、游戏、内容媒体或强视觉表达型页面，可以保留 token、状态、可访问性原则，但字号、图片、布局密度需要单独定义。
- 如果项目没有使用 shadcn、Tailwind 或 Lucide，也应保留同等语义：颜色必须来自 token，状态必须一致，组件尺寸必须有统一 scale。

### 12.2 响应式布局

- 所有核心页面至少验证 mobile、tablet、desktop 三档视口。
- 页面主区域应优先使用流式布局、栅格或 flex wrap，不依赖固定宽度。
- 工具栏在窄屏下应允许换行、收纳到菜单，或降级为图标按钮。
- 表格在窄屏下需要明确策略：横向滚动、列收纳、卡片化三选一，不临时挤压文字。
- 文本不得遮挡其他内容；长标题、长路径、长邮箱、长 ID 必须有换行、截断或 tooltip 策略。

### 12.3 可访问性

- 所有可点击元素必须可通过键盘访问，并有可见的 `focus-visible` 状态。
- 图标按钮必须提供 `aria-label` 或等价的可访问名称。
- 表单错误必须同时有视觉提示和文字说明，不能只靠颜色表达。
- 颜色对比度至少满足 WCAG AA：普通文字 4.5:1，大号文字和图标 3:1。
- Dialog、Dropdown、Popover 需要正确处理焦点进入、焦点返回和 Esc 关闭。
- 禁用状态不能作为解释；如果用户需要知道为什么不能点，应提供内联说明或 tooltip。

### 12.4 内容状态

每个列表、表格、详情页和表单都需要覆盖以下状态：

- Loading：优先使用小型 spinner 或局部 loading 文案。
- Empty：说明当前为空，并给出一个自然的下一步操作。
- Error：说明发生了什么，以及用户可以重试、返回或联系支持。
- Partial：部分数据加载失败时，不要让整个页面空白。
- Permission denied：说明没有权限，而不是伪装成空数据。

### 12.5 表格与列表

- 表格适合比较和批量浏览，卡片适合摘要和行动入口，不要混用造成重复信息。
- 表头使用 `text-xs font-medium text-muted-foreground`，数据行使用 `text-sm`。
- 行 hover 只提示可交互，不代表选中。
- 批量选择、排序、筛选、分页一旦出现，应保持位置稳定，不随数据变化跳动。
- 数字、金额、日期、状态需要统一格式；同一列不要混用不同表达方式。

### 12.6 层级与覆盖物

- Dialog 用于需要用户完成或确认的任务；Popover 用于轻量补充操作；Tooltip 只解释，不承载操作。
- 页面中不要同时打开多个竞争注意力的浮层。
- z-index 必须集中管理，避免在组件里随手写任意值。
- 浮层关闭后，焦点应回到触发它的元素。

### 12.7 文案纪律

- UI 文案优先短句、动词开头、具体可执行。
- 按钮文案写结果，不写抽象动作：例如“创建项目”优于“确定”。
- 删除、覆盖、发布等高风险动作必须写清对象和后果。
- 空状态和错误文案不要责备用户，也不要使用过度情绪化表达。

### 12.8 设计 token 落地

- token 应在一个集中位置维护，并明确 light / dark 两套值。
- 不允许在业务组件中新增一次性颜色、圆角、阴影、字号。
- 新增 token 必须说明语义和使用场景，不能只按颜色命名。
- 组件变体应通过统一的 variant 系统维护，避免每个页面单独拼 class。

### 12.9 最终验收补充

- [ ] mobile / tablet / desktop 是否都验证过？
- [ ] 键盘能否完成核心流程？
- [ ] 图标按钮是否都有可访问名称？
- [ ] 空、加载、错误、无权限状态是否完整？
- [ ] 长文本、长 ID、长邮箱是否不会撑坏布局？
- [ ] 表格列、日期、数字、状态格式是否一致？
- [ ] 浮层焦点、Esc、关闭后焦点返回是否正常？
- [ ] 新增样式是否都来自既有 token 或明确新增的 token？
